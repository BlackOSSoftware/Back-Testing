...
from backtest_mt5 import BacktestConfig, TIMEFRAME_MINUTES, TIMEFRAMES, backtest, build_summary, fetch_rates, parse_date, parse_time, write_outputs
...
app = Flask(__name__, static_folder=None, template_folder=".", instance_relative_config=True)
...
DEFAULT_PASSWORD_HASH = "scrypt:32768:8:1$rtktSCKnHAjel7o5$9f021a038b065866a8a04d9e404e8c8356a81acddc047af1da0e0594de5ac6357b643b65279492a10e1626d9cb73131001ec6a0e44474b548eaaeef401cdf0a0"
...
OPTIMIZER_RUNS_DIR = Path("results") / "optimizer_runs"
...
def default_dates() -> tuple[str, str]:
    return "2025-05-25", "2026-05-25"


def load_latest_summary(source: str = "MT5") -> dict:
    source = normalize_source(source)
    source_path = Path("results") / source.lower() / "backtest_summary.json"
    path = source_path if source_path.exists() else Path("results") / "backtest_summary.json"
...
def highcharts_asset(filename: str):
    ...
    return send_from_directory("vendor/highcharts", filename)
...
def symbols():
    ...
    names = sorted({symbol.name for symbol in all_symbols})
...
def history_range():
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    symbol = request.args.get("symbol", "BTCUSDc").strip()
...
def run_backtest():
    ...
    try:
        ...
        config = BacktestConfig(
            ...
            entry_buffer_pct=float(data.get("entry_buffer_pct", 0.05)) / 100,
...
def static_files(path: str):
    if path == "report_data.js" and Path(path).is_file():
